#!/usr/bin/env bash
#
# Script to create a macOS dmg file for Rings3D builds, including code
...
_volume_name="RINGS 3D"
_tmp_dir="$(mktemp -d)"
_tmp_dmg="/tmp/rings-tmp.dmg"
_background_image="${_script_dir}/ringsce.png"
...
if [ ! -d "${SRC_DIR}/Rings3D.app" ]; then
    echo "use --source parameter to set source directory where Rings3D.app can be found"
    exit 1
fi
...
if [ -d "${_mount_dir}" ]; then
    echo -n "Ejecting existing Rings3D volume.."
...
# Copy dmg contents.
echo -n "Copying Rings3D.app..."
cp -r "${SRC_DIR}/Rings3D.app" "${_tmp_dir}/" || exit 1
echo
...
cat "${_script_dir}/rings.applescript" | osascript
...
if [ ! -z "${C_CERT}" ]; then
    ...
    for f in $(find "${_mount_dir}/Rings3D.app/Contents/Resources" -name "python*"); do
        if [ -x ${f} ] && [ ! -d ${f} ]; then
    ...
    for f in $(find "${_mount_dir}/Rings3D.app" -name "*.dylib" -o -name "*.so"); do
        codesign --remove-signature "${f}"
        codesign --timestamp --options runtime --entitlements="${_entitlements}" --sign "${C_CERT}" "${f}"
    done
    echo ; echo -n "Codesigning Rings3D.app"
    codesign --remove-signature "${_mount_dir}/Rings3D.app"
    codesign --timestamp --options runtime --entitlements="${_entitlements}" --sign "${C_CERT}" "${_mount_dir}/Rings3D.app"
    echo
else
